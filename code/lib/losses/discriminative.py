...
def calculate_means(pred, gt, n_objects, max_n_objects, usegpu, eps = 1e-10):
    ...
    for i in range(bs):
        ...
        _mean_sample = _pred_masked_sample.sum(
            0) / (_gt_expanded_sample.sum(0) +eps)  # n_objects, n_filters
...
def calculate_variance_term(pred, gt, means, n_objects, delta_v, norm=2, eps = 1e-10):
    """pred: bs, height * width, n_filters
       gt: bs, height * width, n_instances
    ...
    for i in range(bs):
        ...
        var_term += torch.sum(_var_sample) / (torch.sum(_gt_sample)+eps)
    var_term = var_term / (bs  + eps)
 
    return var_term


def calculate_distance_term(means, n_objects, delta_d, norm=2, usegpu=True, eps = 1e-10):
    """means: bs, n_instances, n_filters"""
    ...
    dist_term = dist_term / (bs + eps)

    return dist_term


def calculate_regularization_term(means, n_objects, norm, eps = 1e-10):
    """means: bs, n_instances, n_filters"""
